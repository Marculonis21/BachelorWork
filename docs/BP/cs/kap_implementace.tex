...
zlepšení čitelnosti a zjednodušení rozšiřování kódu, kde nyní každý modul
    ...
    \item metody využívané evolučním algoritmem:
        \begin{itemize}
            ...
            \item \texttt{switch\_evo\_phase()} -- metoda využívána z hlavního
                modulu, který řídí evoluční algoritmus, sloužící pro zahlášení
...
\begin{itemize}
    ...
    \item \textbf{\emph{SineFuncFullAgent}} -- agent, jehož genotyp popisuje
        parametry sinus funkce pro každý aktuátor robota (amplituda,
...
\begin{itemize}
    ...
    \item \texttt{uniform\_shift\_mutation(pop, agent)} -- implementace
        ...
        změny z povoleného rozsahu, kterou přičte k původní mutované hodnotě.
\end{itemize}
...
Pokud máme vytvořenou konfiguraci robota s korektním XML formátem v souboru 
(např. soubor \emph{custom\_stick\_ant.xml}), můžeme ho jednoduše přidat mezi
...
Naše knihovna umožňuje jednoduché přidání robotů, kteří jsou volně dostupní z
knihovny Gymnasium. Většina těchto robotů má ve svém prostředí definované
složitější cíle, kterých mají dosáhnout (např. balancování kyvadla ve
vertikální poloze). Tato prostředí jsou často smysluplně využitelná pouze pro
experimenty s vývoj pomocí algoritmů neuroevoluce. Několik vybraných robotů
jsou jako ukázky již implementováni v naší knihovně -- \emph{Walker2D},
\emph{InvertedPendulum}, \emph{InvertedDoublePendulum}.
