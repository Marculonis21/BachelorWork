...
jako shodné (\emph{matching genes}), nebo se neshodují. Neshodné geny, které
mají historické značky v rozmezí hodnot \emph{historických značek} druhého z
jedinců, se nazývají disjunktní (\emph{disjoint}) a zbylé geny se nazývají
přesahující (\emph{excess}). Hodnota vzdálenosti dvou jedinců se poté počítá
jednoduchou lineární kombinací počtu genů jako:
